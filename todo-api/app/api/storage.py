...
# BEGIN git uri handshake endpoints
class ResolveURIRequest(BaseModel):
    uri: str

class FromAbsRequest(BaseModel):
    abs_path: str

@router.get('/git/config')
def get_git_config():
    """Git storage config for path mapping handshake."""
    return {
        "uri_scheme": "git",
        "uri_prefix": "git://",
        "git_root": str(settings.get_git_repo_path()),
...
@router.post('/git/resolve-uri')
def resolve_git_uri(req: ResolveURIRequest):
    """Resolve git:// relative URI to absolute server path."""
    prefix = "git://"
    if not req.uri.startswith(prefix):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Invalid git URI")
    rel = req.uri[len(prefix):]
    git_root = settings.get_git_repo_path().resolve()
    abs_path = (git_root / rel).resolve()
    try:
        abs_path.relative_to(git_root)
    except ValueError:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Path outside git root")
    return {"uri": req.uri, "abs_path": str(abs_path), "git_root": str(git_root)}

@router.post('/git/from-abs')
def git_uri_from_abs(req: FromAbsRequest):
    """Convert absolute server path under git root to git:// relative URI."""
    git_root = settings.get_git_repo_path().resolve()
    abs_path = Path(req.abs_path).resolve()
    try:
        rel = abs_path.relative_to(git_root)
    except ValueError:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail="Path outside git root")
    return {"abs_path": str(abs_path), "uri": f"git://{rel.as_posix()}", "git_root": str(git_root)}


# BEGIN fs uri handshake endpoints
@router.get('/fs/config')
def get_fs_config():
    """file_storage_root の設定（相対パス共有のための最小情報）。"""
    return {
        "uri_scheme": "fs",
        "uri_prefix": "fs://",
        "fs_root": str(settings.get_file_storage_root()),
        "version": 1,
    }

@router.post('/fs/resolve-uri')
def resolve_fs_uri(req: ResolveURIRequest):
    """fs:// 相対URI → サーバ絶対パス。"""
    prefix = 'fs://'
    if not req.uri.startswith(prefix):
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Invalid fs URI')
    rel = req.uri[len(prefix):]
    fs_root = settings.get_file_storage_root().resolve()
    abs_path = (fs_root / rel).resolve()
    try:
        abs_path.relative_to(fs_root)
    except ValueError:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Path outside fs root')
    return {"uri": req.uri, "abs_path": str(abs_path), "fs_root": str(fs_root)}

@router.post('/fs/from-abs')
def fs_uri_from_abs(req: FromAbsRequest):
    """サーバ絶対パス（file_storage_root配下）→ fs:// 相対URI。"""
    fs_root = settings.get_file_storage_root().resolve()
    abs_path = Path(req.abs_path).resolve()
    try:
        rel = abs_path.relative_to(fs_root)
    except ValueError:
        raise HTTPException(status_code=status.HTTP_400_BAD_REQUEST, detail='Path outside fs root')
